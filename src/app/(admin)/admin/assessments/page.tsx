...
import {
  ...
  Link2, ChevronDown, ChevronUp, Upload, BarChart3, Clock, Users,
...
interface AssessmentItem {
  ...
  departments: string[]; batchIds: string[]; status: string;
  deadline: string | null; maxScore: number | null; createdAt: string;
...
interface DetailedAssessment {
  ...
  links: LinkItem[]; submissions: SubmissionItem[];
}
...
export default function AdminAssessmentsPage() {
    ...
    (async () => {
      try {
        const [, dRes] = await Promise.all([
          fetchAssessments(),
          adminApi.listDepartments() as any,
        ]);
  ...
  const fetchDetail = async (id: string) => {
    ...
    try {
      const [dRes, sRes] = await Promise.all([
        adminApi.getAssessment(id) as any,
        adminApi.getAssessmentStats(id) as any,
      ]);
  ...
  const handleExcelUpload = async (id: string, file: File) => {
    ...
    try {
      ...
      if (lines.length < 2) { showToast("error", "Excel/CSV must have header + data rows"); setUploadingGrades(false); return; }
      const headers = lines[0].split(",").map(h => h.trim().toLowerCase());
      const usnIdx = headers.findIndex(h => h === "usn" || h === "roll" || h === "roll_no" || h === "rollno");
      const scoreIdx = headers.findIndex(h => h === "score" || h === "marks" || h === "total");
      const remarksIdx = headers.findIndex(h => h === "remarks" || h === "comment" || h === "comments");
      ...
      const grades = lines.slice(1).map(line => {
        ...
        return {
          ...
          remarks: remarksIdx >= 0 ? cols[remarksIdx] || undefined : undefined,
        };
      }).filter(g => g.usn);
      const res = await adminApi.bulkGradeAssessment(id, grades) as any;
      const d = res?.data;
      showToast("success", `Graded ${d?.graded || 0} students. ${d?.notFound || 0} USNs not found.`);
  ...
  return (
    <div className="page-enter">
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-24 sm:pb-10 -mt-2 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <select value={filterType} onChange={e => setFilterType(e.target.value)}
            className="px-3 py-2 bg-white border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
          ...
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
            ...
            {["draft", "active", "expired", "archived"].map(s => <option key={s} value={s} className="capitalize">{s}</option>)}
          </select>
          <div className="flex-1" />
          <button onClick={() => setShowCreate(true)}
            className="flex items-center gap-1.5 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl text-sm font-medium shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 transition-all">
                ...
                <div key={a.id} className={cn("i-card p-4 cursor-pointer hover:shadow-md transition-all border-l-4",
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-semibold border capitalize", TYPE_COLORS[a.type] || TYPE_COLORS.custom)}>{a.type}</span>
                        <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize", STATUS_COLORS[a.status] || STATUS_COLORS.draft)}>{a.status}</span>
                  ...
                  {a.deadline && (
                    <p className={cn("text-[10px] mt-1.5", isExpired ? "text-red-500" : "text-muted-foreground")}>
                      ...
                      {isExpired ? "Expired" : `Deadline: ${new Date(a.deadline).toLocaleDateString()}`}
                    </p>
                  )}
                </div>
              );
        ...
        {/* ── Detail Panel ── */}
        {detailId && (
          <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-8 sm:pt-16 px-4" onClick={() => { setDetailId(null); setDetail(null); }}>
                ...
                <div className="p-5 sm:p-6 space-y-5">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={cn("px-2 py-0.5 rounded-full text-xs font-semibold border capitalize", TYPE_COLORS[detail.type])}>{detail.type}</span>
                    ...
                    <div className="flex items-center gap-2">
                      {detail.status === "draft" && (
                        <button onClick={() => handleStatusChange(detail.id, "active")}
                      ...
                      {detail.status === "active" && (
                        <button onClick={() => handleStatusChange(detail.id, "expired")}
                          className="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-xs font-medium hover:bg-amber-600 transition-colors">
                          Expire
                        </button>
                      )}
                      <button onClick={() => handleDelete(detail.id)} className="p-1.5 text-red-400 hover:text-red-600 transition-colors">
                        <Trash2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => { setDetailId(null); setDetail(null); }} className="p-1.5 text-muted-foreground hover:text-foreground">
                        <X className="w-5 h-5" />
                      </button>
                    </div>
                  </div>
                  ...
                  {stats && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      ...
                      <div className="p-3 bg-amber-50 rounded-xl text-center">
                        ...
                        <p className="text-[10px] text-amber-500 font-medium">Avg Score</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
                      <Link2 className="w-4 h-4" /> Test Links ({detail.links.length})
                    </h3>
                          ...
                          <div key={l.id} className="flex items-center justify-between p-2.5 bg-accent/30 rounded-xl">
                            ...
                            <div className="flex items-center gap-1.5 ml-2">
                              ...
                              <button onClick={async () => {
                                await adminApi.removeAssessmentLink(l.id);
                  ...
                  <div className="p-4 bg-gradient-to-br from-indigo-50 to-violet-50 rounded-xl border border-indigo-100">
                    <h3 className="text-sm font-semibold text-foreground mb-1 flex items-center gap-1.5">
                      <Upload className="w-4 h-4" /> Upload Scores (CSV)
                    </h3>
                    <p className="text-[11px] text-muted-foreground mb-3">
                      CSV with columns: <strong>USN, Score</strong> (optional: Remarks). Students with scores = attended.
                    </p>
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
                      <Users className="w-4 h-4" /> Submissions ({detail.submissions.length})
                    </h3>
                    {detail.submissions.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No students assigned yet. Publish the assessment to assign students.</p>
                    ) : (
                              ...
                              <tr key={s.id} className="border-b border-border/50 hover:bg-accent/20 transition-colors">
                                ...
                                <td className="py-2 px-2 text-center">
                                  <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize",
                                    ...
                                    s.status === "absent" ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-600")}>
                                    {s.status}
                                  </span>
                                </td>
                                <td className="py-2 px-2 text-center font-bold text-foreground">
                                  {s.score != null ? `${s.score}${detail.maxScore ? `/${detail.maxScore}` : ""}` : "—"}
                                </td>
                              </tr>
                            ))}
                          </tbody>
                  ...
                  {stats?.departmentStats?.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
                        <BarChart3 className="w-4 h-4" /> Department Breakdown
                      </h3>
      ...
      {/* Toast */}
      {toast && (
        <div className={cn("fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] px-5 py-3 rounded-xl shadow-lg text-sm font-medium flex items-center gap-2 animate-in slide-in-from-bottom-4",
...
function CreateModal({ departments, onClose, onCreated }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-6 sm:pt-12 px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-5 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Aptitude Test - Batch 2026"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={type} onChange={e => setType(e.target.value)}
                className="w-full px-3 py-2.5 bg-white border border-border rounded-xl text-sm capitalize focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
                {TYPES.map(t => <option key={t} value={t} className="capitalize">{t}</option>)}
              </select>
            </div>
            <div>
          ...
          {/* Departments */}
          <div>
            <label className="text-xs font-semibold text-muted-foreground block mb-1.5">Departments (select target)</label>
            <div className="flex flex-wrap gap-1.5">
              {departments.map(d => (
                <button key={d.code} onClick={() => toggleDept(d.code)}
                  className={cn("px-2.5 py-1 rounded-lg text-xs font-medium border transition-all",
                    selectedDepts.includes(d.code)
          ...
          <div>
              ...
              {links.map((l, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input value={l.title} onChange={e => { const n = [...links]; n[i].title = e.target.value; setLinks(n); }}
                    placeholder="Link title" className="flex-1 px-2.5 py-2 bg-white border border-border rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/30" />
                  <input value={l.url} onChange={e => { const n = [...links]; n[i].url = e.target.value; setLinks(n); }}
                    placeholder="https://..." className="flex-[2] px-2.5 py-2 bg-white border border-border rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/30" />
            ...
            <button onClick={() => setLinks([...links, { title: "", url: "", platform: "custom" }])}
              className="mt-2 text-xs text-indigo-600 font-medium hover:text-indigo-700 flex items-center gap-1">
