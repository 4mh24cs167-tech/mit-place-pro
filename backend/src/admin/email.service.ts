...
export class EmailService {
  ...
  private async initTransporter() {
    ...
    if (smtpUser && smtpPass) {
      ...
      this.logger.log('SMTP initialized with provided credentials.');
    } else {
      this.logger.warn('No SMTP credentials found. Generating Ethereal test account...');
      ...
      }).catch(err => this.logger.error('Failed to create Ethereal account', err));
    }
  }
  ...
  private logPreviewUrl(info: any) {
    ...
    if (url) {
      this.logger.log('📧 Email preview available at: ' + url);
    }
  }
